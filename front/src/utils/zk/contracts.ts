...
export async function createGame(firstLocation: number) {
	...
	const first_hash = first_poseidon.F.toString(first_poseidon([0, first_location]));
	return {
		first_hash: first_hash,
	};
}

export async function join_and_make_move(values: JoinMoveValues, shot: number) {
	const { previous_location, move, new_location } = values;

	let poseidon = await circomlibjs.buildPoseidon();
	const previous_commit_hash = poseidon.F.toString(poseidon([0, previous_location]));
...
export type MoveValues = JoinMoveValues & {
	enemy_shot: number;
	previous_commit_hash: string;
	hit: boolean;
};

export type JoinMoveValues = {
...
export async function make_move(values: MoveValues, shot: number): Promise<MoveArgs> {
	const { previous_location, previous_commit_hash, move, new_location, hit, enemy_shot } = values;
